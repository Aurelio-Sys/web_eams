...
<tr class="foottr">
  ...
  <td class="foot1" data-label="Asset" style="text-align: right">{{ number_format($show->asset_meter,0) }}</td>
  ...
  @if(is_null($show->asset_last_usage))
    0
  @else
    {{ number_format($show->asset_last_usage,0) }}
  @endif
  </td>
  <td class="foot1" data-label="Next Usage" style="text-align: right">{{ number_format($show->asset_last_usage + $show->asset_meter,0) }}</td>
