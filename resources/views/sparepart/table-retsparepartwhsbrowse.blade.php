...
<tr>
    ...
    @if($datas->ret_sp_status == 'open' || $datas->ret_sp_status == 'partial transferred')
    <a class="btn btn-success" href="{{route('retspwhsdet', $datas->ret_sp_number)}}"><i class="fa fa-check-circle"></i> Confirm</a>
    @else
    ...
    @endif
    <a class="btn btn-info histtrfsp" style="padding: 0.4em 1.2em;" href="javascript:void(0)"
    data-rsnumber="{{ $datas->ret_sp_number }}" data-reqby="{{ $datas->ret_sp_return_by }}" data-trfby="{{ $datas->ret_sp_transfered_by }}" data-trfdate="{{ $datas->ret_sp_transfer_date == '0000-00-00' ? '' : date('m/d/Y', strtotime($datas->ret_sp_transfer_date)) }}" data-status="{{ $datas->ret_sp_status }}"
    data-spcode="{{ $datas->ret_spd_sparepart_code }}" data-spdesc="{{ $datas->spm_desc }}" data-qtyreq="{{ $datas->ret_spd_qty_return }}" data-qtytrf="{{ $datas->ret_spd_qty_transfer }}"
    data-locfrom="{{ $datas->ret_spd_loc_from }}" data-sitefrom="{{ $datas->ret_spd_site_from }}" data-lotto="{{ $datas->ret_spd_lot_to }}" data-locto="{{ $datas->ret_spd_loc_to }}" data-siteto="{{ $datas->ret_spd_site_to }}" data-note="{{ $datas->ret_spd_whsnote }}"
