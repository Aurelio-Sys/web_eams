...
class HomeController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        $totsr = DB::table('service_req_mstr')
            ...
            ->selectRaw("count(case when DATEDIFF(sr_created_at, '$tglnow') <= 3 then 1 end) as a")
            ->selectRaw("count(case when DATEDIFF(sr_created_at, '$tglnow') > 3 and DATEDIFF(sr_created_at, '$tglnow') <= 7 then 1 end) as b")
            ->selectRaw("count(case when DATEDIFF(sr_created_at, '$tglnow') > 7 and DATEDIFF(sr_created_at, '$tglnow') <= 14 then 1 end) as c")
            ->selectRaw("count(case when DATEDIFF(sr_created_at, '$tglnow') > 14 then 1 end) as d")
            ->first();
            
        $totwo = DB::table('wo_mstr')->where('wo_status', 'open')
            ->selectRaw('count(*) as total')
            ->selectRaw("count(case when DATEDIFF(wo_created_at,'$tglnow') <= 3 then 1 end) as a1")
            ->selectRaw("count(case when DATEDIFF(wo_created_at,'$tglnow') > 3 and DATEDIFF(wo_created_at,'$tglnow') <= 7 then 1 end) as b1")
            ->selectRaw("count(case when DATEDIFF(wo_created_at, '$tglnow') > 7 and DATEDIFF(wo_created_at, '$tglnow') <= 14 then 1 end) as c1")
            ->selectRaw("count(case when DATEDIFF(wo_created_at,'$tglnow') > 14 then 1 end) as d1")
            ->first();
